...
export const getRolPrograma = async (ci) => {
  const accessToken = localStorage.getItem("token");
  const url = `http://localhost:5000/participantes/rol-programa/${ci}`;
  try {
    const response = await fetch(url, {
      method: "GET",
      headers: {
    ...
    if (response.ok) {
      ...
      throw new Error(`Fallo al obtener el rol: ${errorData.message}`);
    }
  } catch (error) {
    console.error("Error de red o del servidor:", error);
    throw error;
